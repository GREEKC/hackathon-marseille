...
author: "Yvon Mbouamboua, Pascal Rihet, Andrew Parton & Jacques van Helden "
date: '`r Sys.Date()`'
output:
  html_document:
    code_folding: hide
...
for (rsID in DArsIDs) {
  ...
  message("\tFound\t",  length(newLdSNpIDs), " LD SNPs for DA-SNP ", i, "/", nbrsIDs, "\t", rsID)
  # sView(ldSNPs)
  nbNewSNPIDs <- length(newLdSNpIDs)
  if (nbNewSNPIDs > parameters$max.ld.per.snp) {
    message("Number of LD for tag SNP ", rsID, " (", nbNewSNPIDs,
            ") exceeds limit (", parameters$max.ld.per.snp, "). LD SNPs are ignored for this Tag SNP. ")
...
# haploreg.path <- file.path(result.dir.path["HaploReg"], haploreg.file)
...
SOIsBed <- data.frame(chrom = SOI$chr_name,
                      chromStart = SOI$chrom_start - 0, # BEWARE: in bed the first chrom position is 0
...
# Run biomaRt
LdSNPsInfo <- getSNP(rs = LDsnpIDs, snpmart)
LdSNPsInfo <- LdSNPsInfo[!duplicated(LdSNPsInfo$refsnp_id),]

# Select the non-coding variant by discarding the variants with an annotated CDS start
isLDSOI <- is.na(LdSNPsInfo$cds_start)
# table(isSOI)
LDSOI <- LdSNPsInfo[isLDSOI, ]
LDSOI$chr_name <- paste("chr", LDSOI$chr_name, sep = "")
LDSOI <- LDSOI[, c("chr_name", "chrom_start", "chrom_end","refsnp_id")]
View(LDSOI)


# haplotype <- merge(x = LDSOI,
#                    y = newLdSNPs,
#                    by.x = "refsnp_id",
#                    by.y = "variation2",
#                    all.x = TRUE,
...
varscan.path <- file.path(result.dir.path["RSAT"], varscan.file)
