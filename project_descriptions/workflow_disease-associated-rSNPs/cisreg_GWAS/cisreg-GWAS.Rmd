...
#  View(studyRESToutput)
  DAsnpIDs <- append(DAsnpIDs, newSnpTable$rsId)
}
...
## Check SNP IDs
rsIDs <- grep(pattern = '^rs\\d+', DAsnpIDs, perl = TRUE, value = TRUE)

nonRsIDs <- setdiff(DAsnpIDs, rsIDs)
message("SNPs with rs identifier: ", length(rsIDs))
...
for (rsID in rsIDs) {
  ...
  ldURL <- paste(sep = "", 
                 ...
                 "&r2=", parameters$r2, 
                 "&window_size=", parameters$ld.distance)
  newLdSNPs <- fromJSON(ldURL, simplifyDataFrame = TRUE)
  # names(newLdSNPs)
  lsSNPsIDs <- append(lsSNPsIDs, newLdSNPs$variation2)
  message("\tCollected\t",  nrow(newLdSNPs), " LD SNPs for DA-SNP ", i, "/", nbRsIDs, "\t", rsID)
  #View(ldSNPs)
}
