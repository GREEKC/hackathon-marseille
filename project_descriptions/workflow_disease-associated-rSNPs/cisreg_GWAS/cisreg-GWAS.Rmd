...
parameters <- list(
  ...
  query = "EFO_0001359", # type ii diabetes mellitus
  ...
  RsatRestUrl = "http://rsat-tagc.univ-mrs.fr/rest/",
...
We define as **tag SNPs** all SNPs (coding and non-coding) associated to the disease. 
...
SOIsConsequenceTable <- table(snpInfo$consequence_type_tv)
...
We define as **SNPs of Interest** (**SOIs**) all the non-coding SNPs that are either associated to the disease, and in LD with these associated SNPs. 
...
# Transform TF names to uppercase

varScanJasparTF$name <- paste(toupper(varScanJasparTF$name))
...
# Specify the directory results
remapRes <-  paste(sep = "", parameters$data.dir, "/remap2018_all_macs2_hg38_v1_2.bed.gz")

# Downloading the ReMap peak data 
#download.file(parameters$remapUrl, destfile = remapRes, method = "libcurl")

if (!file.exists(remapRes)) {
  message ("Downloading")
  download.file(parameters$remapUrl, dest = remapRes, method = "libcurl")
} else {
  message ("remap2018_all_macs2_hg38_v1_2.bed.gz ", "already exists, skipping download. ")
}
...
if (!exists("remap.data")) {
  message ("Reading")
  remap.data <- fread(file = remapRes)
} else {
  message ("remap.data", "already read, skipping reading. ")
}

#head(remap.data)

# make a grange object of remap data
#remapCatalog <- ReMapEnrich::bedToGranges(remap.data)

if (!exists("remapCatalog.gr")) {
  message ("Make grange object")
  remapCatalog.gr <- with(remap.data,
                   GRanges( seqnames = Rle(V1),
                            ranges   = IRanges(start = V2, end = V3),
                            strand   = Rle("*"), name = V4))
} else {
  message ("remapCatalog.gr", "already exists, skipping make grange object. ")
}

#bedToGranges(remap.data)


# Define the path to saving .Rda file
rda.file <- file.path(parameters$data.dir,
                      paste(sep = "", "remap2018_all_macs2_hg38_v1_2", ".Rda"))

# Save the remap data Grange format in Rda format
if (!file.exists(rda.file)) {
  message ("Saving ReMap file in Rda format")
  save(remapCatalog, file = rda.file)
} else {
  message ("remap2018_all_macs2_hg38_v1_2.Rda ", " already exists, skipping saving. ")
}

# Load the ReMap Rda file
load(rda.file)


# Create a BED file of snpInfo data
# SOIsBed <- snpInfo[, c("chr_name", "chrom_start", "chrom_end", "refsnp_id")]
# SOIsBed$chr_name <- paste("chr", SOIsBed$chr_name, sep = "")
...
remap.intersect.SOIs <- data.frame(IntersectBed(remapCatalog, haplotype.block.gr))
...
# Histogramme of TFs
hist(sort(table(remap.intersect.SOIs$id), decreasing = TRUE), 
        names.arg = "",
...
```{r remapenrichment}
# Enrichment of TFs
enrich <- enrichment(haplotype.block.gr, remapCatalog,  byChrom = TRUE)  
#enrich <- enrichment(haplotype.block.gr, remapCatalog) 
#enrich <- enrichment(haplotype.block.gr, remapCatalog, byChrom = TRUE)
head(enrich)
...
enrichment <- file.path(
  result.dir.path["ReMap"], 
  paste(sep = "", parameters$query, "_ReMap_enrichment", ".tsv"))

write.table(x = enrich,
              file = enrichment,
              quote = FALSE,
              sep = "\t",
              row.names = FALSE,
...
# Negative control
shufflesRegion <- shuffle(haplotype.block.gr,  byChrom = TRUE)
#df <- data.frame(shufflesRegion)

#enrichNeg <- enrichment(shufflesRegion, remapCatalog, fractionQuery = 0, fractionCatalog = 0)
enrichNeg <- enrichment(shufflesRegion, remapCatalog, fractionQuery = 0, fractionCatalog = 0)
#enrichNeg  <- enrichment(shufflesRegion, remapCatalog, byChrom = TRUE)


enrichmentNeg <- file.path(
  result.dir.path["ReMap"], 
  paste(sep = "", parameters$query, "_ReMap_enrichment_neg_control", ".tsv"))

write.table(x = enrichNeg,
              file = enrichmentNeg,
              quote = FALSE,
              sep = "\t",
              row.names = FALSE,
...
enrichmentBarPlot(enrich, 
                  #sigDisplayQuantile = 0.5, 
                  top = 20, 
                  aRisk = 0.00001)


# Display a bar plot
enrichmentBarPlot(enrichNeg, 
                  #sigDisplayQuantile = 0.5, 
...
#par(mfrow = c(1, 2))
# Display a dot plot.
enrichmentDotPlot(na.omit(enrich), 
                  top = 20,
...
enrichmentDotPlot(na.omit(enrichNeg), 
                  top = 20,
                  main = paste("Negative control"),
                  sigType = "q",
                  col = c("#6699ff", "#ff5050"),
