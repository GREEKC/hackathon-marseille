...
tagSNPsInfo$consequence_type_tv <- sub(pattern = " ", NA, 
                                       tagSNPsInfo$consequence_type_tv)
...
We select the **SOIs** by keeping all **LD SNPs in non-coding regions** and we checking these SNPs in dbSNP using `biomaRt package`. We are a total **`r length(SOI$refsnp_id)` SOIs.**
...
table <- table(SOI$consequence_type_tv)
pct <- round(table/sum(table)*100)
lbls <- names(table)
lbls <- paste(lbls, pct) # add percents to labels
lbls <- paste(lbls, "%", sep = "") # ad % to labels
...
We used the `xEnricherSNPs` function to conduct enrichment analysis given a list of SNPs and the ontology (Experimental Factor Ontology used to annotate GWAS Catalog SNPs). in query and return an object of class "eTerm". The enrichment analysis is based on either "Fisher's" exact test or Hypergeometric test.
...
eTerm <- xEnricherSNPs(SOI$refsnp_id, 
                       ontology = c("EF", 
                                    "EF_disease",
                                    "EF_phenotype", 
                                    "EF_bp"), 
...
```{r rsat_rest_api_R}
variant <- data.frame(SOI$refsnp_id)
names(variant) <- NULL
vv <- as.factor(variant[1:2, ])

#data <- c("rs59576053", "rs73409643")
## Variation-info
message("Get variation information from RSAT")
res <- POST("http://rsat-tagc.univ-mrs.fr/rest/variation-info", 
            body = list(
              species = "Homo_sapiens", 
              assembly = "GRCh38", 
              i = vv))
              #i = upload_file("~/Google Drive/Yvon_Mbouamboua_these_2018/rest/file.txt")))

var <- content(res, "text", encoding = "UTF-8")

varinfo <- fromJSON(var)

#varBed <- varinfo$output
varBed <- varinfo$result_path


write.table(x = varinfo$output,
            file = "~/Google\ Drive/Yvon_Mbouamboua_these_2018/rest/varBed",
            quote = FALSE,
            sep = "\t",
            row.names = FALSE,
            col.names = TRUE)

# varBed <- read.delim(file = "~/Google\ Drive/Yvon_Mbouamboua_these_2018/rest/varBed",
#                        header = TRUE,
#                        sep = "\t",
...
seq <- POST("http://rsat-tagc.univ-mrs.fr/rest/retrieve-variation-seq", 
            body = list(
              species = "Homo_sapiens", 
              assembly = "GRCh38", 
              i_input_string = varBed,
              i_input_string_type = 'piping',
              mml = 30,
              format =  "varBed"))

sequence <- content(seq, "text", encoding = "UTF-8")
retrieveSeq <- fromJSON(sequence)
varSeq <- retrieveSeq$result_path


## Variation-scan
varscan <- POST("http://rsat-tagc.univ-mrs.fr/rest/variation-scan", 
            body = list(
              species = "Homo_sapiens", 
              assembly = "GRCh38", 
              i_input_string = varSeq,
              i_input_string_type = 'piping',
              m = upload_file("~/Google\ Drive/Yvon_Mbouamboua_these_2018/rest/JASPAR2018_CORE_vertebrates_non-redundant_pfms_transfac.txt"),
              m_format =  "transfac"),
            #add_headers("Accept" = "application/json"))
add_headers("Accept" = "text/plain"))

var <- content(varscan, "text", encoding = "UTF-8")
#scan <- fromJSON(var, simplifyDataFrame = TRUE)
print(var)
dd <- data.frame(var)
...
remap.intersect.SOIs <- IntersectBed(remapCatalog, SOIsBed.gr)
...
# Enrichment of TFs
enrich <- enrichment(SOIsBed.gr, remapCatalog, fractionQuery = 0, fractionCatalog = 0)  
...
```{r enrichment_barplot_volcanoplot}
