...
haplotype$chr_name <- paste("chr", haplotype$chr_name, sep = "")
haplotype <- haplotype[, c("chr_name", "chrom_start", "chrom_end","refsnp_id", "allele", "consequence_type_tv",  "variation1", "r2", "d_prime","population")]
haplotype <- na.omit(haplotype)

haplotype$chrom_start <- as.numeric(haplotype$chrom_start)
...
LD <- haplotype[, c("chr_name", "chrom_start", "refsnp_id", "variation1")]
LD <- na.omit(LD)
LD$chrom_start <- as.numeric(LD$chrom_start)
LDgr <- with(LD,
GRanges( seqnames = Rle(chr_name),
ranges   = IRanges(start = chrom_start, end = chrom_start),
strand   = Rle("*"), idLd = refsnp_id, idTag = variation1))
