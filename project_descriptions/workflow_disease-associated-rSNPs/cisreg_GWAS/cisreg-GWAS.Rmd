...
author: "Yvon Mbouamboua, Pascal Rihet, Andrew Parton, Aziz Khan & Jacques van Helden "
date: '`r Sys.Date()`'
output:
  html_document:
    code_folding: hide
...
```{r export_tag_snps, eval=FALSE, echo=FALSE}

## NOT WORKING FOR THE TIME BEING, DUE TO LISTS IN THE TABLE COLUMNS

# Export missing data in TSV table
gwas.err.file <- "GWAS_noRSID.tsv"
gwas.err.path <- file.path(result.dir.path["TagSNPs"], gwas.err.file)
...
# write.table(x = as.data.frame(subset(snpTable, rsId %in% nonrsIDs)),
#             file = gwas.err.path,
#             quote = FALSE,
#             sep = "\t",
#             row.names = FALSE,
...
# tag.snp.path <- file.path(result.dir.path["TagSNPs"], tag.snp.file)
# outfiles <- rbind(outfiles, data.frame(
#   name = "Tag SNPs", 
#   file = tag.snp.file, 
#   path = tag.snp.path,
...
In total, we found `r print("TO BE ADAPTED")` **TagSNPs** from GWAS catalog.


# Proportion of genomic context

```{r fig.cap = paste("Genomic context of disease-associated variants from GWAS"), out.width = "100%", eval=FALSE, echo=FALSE}
...
## TO DO : replace newLdSNPs by LdSnps once we have fixed the fusion problem
kable(head(newLdSNPs), caption = "Variant in Linkage disequilibrium with Tag SNPs within the window size of set to 200 kb")


## TO DO

# - merge DASNPs and LD-SNPs
# - filter out coding SNPs -> SNPs of interest (SOIs)
# - gather all the information (coordinates, ...) for SOIs
# ...
```


## Genomic features of SNPs in linkage disequilibrium

In this section, we gathering the informations about LD SNPs from Ensembl using `biomaRt package`.
...
getSNP <- function(rs, mart = snpmart) {
...
# Run biomaRt
snpInfo <- getSNP(rs = rsIDs, snpmart)
 #View(snpInfo)
...
# pie(table(snpInfo$consequence_type_tv))

# dim(snpInfo)

# Select the non-coding variant by discarding the variants with an annotated CDS start
isSOI <- is.na(snpInfo$cds_start)
# table(isSOI)
SOI <- snpInfo[isSOI, ]
dim(SOI)

SOI.file <- "snps_of_interest.tsv"
nocoding.path <- file.path("~/cisreg-GWAS_results/EFO_0008407/", SOI.file)
write.table(x = SOI,
            file = nocoding.path,
            quote = FALSE,
            sep = "\t",
...
```{r enrich_association_variant_set, eval=FALSE, echo=FALSE}
...
```{r biomart_annotation, eval=FALSE, echo=FALSE}
...
# 
# attach(haploR)
# SOI <- haploR[which(dbSNP_functional_annotation == "INT" |
...
# detach(haploR)
# 
# SOI$ENSG <- gsub('\\..+$', '', SOI$GENCODE_id)
# 
# # Export SOIs full results in TSV table
# haplor.file <- "ENSG.txt"
# haplor.path <- file.path(result.dir.path["HaploReg"], haplor.file)
# write.table(x = unique(SOI$GENCODE_name),
#             file = haplor.path,
#             quote = FALSE,
#             sep = "\t",
#             row.names = FALSE,
...
SOIsBed <- data.frame(chrom = SOI$chr_name,
                      chromStart = SOI$chrom_start - 1, # BEWARE: in bed the first chrom position is 0
                      chromEnd = SOI$chrom_end, # BEWARE: in bed the end position is the first position after the feature, in 0-based coordinates
                      name = SOI$refsnp_id)
...
# message("The following DAsnpIDs had no information available on dbSNP:  ",
#               paste( biomaRtError$rsID , collapse = ", "),
#               call. = FALSE)


# TO DO :; Export SOIs full results in TSV table


# haplor.file <- "SOIs.tsv"
# haplor.path <- file.path(result.dir.path["SOIs"], haplor.file)
# write.table(x = SOIs,
#             file = haplor.path,
#             quote = FALSE,
#             sep = "\t",
...
# Proportion of genomic context
## TO DO: adapt
SOI$dbSNP_functional_annotation <- sub(pattern = "^.$", "intergenic", 
                                       SOI$consequence_type_tv)
SOI$dbSNP_functional_annotation <- sub(pattern = "^INT$", "intron", SOI$dbSNP_functional_annotation)
SOI$dbSNP_functional_annotation <- sub(pattern = "^U3$", "3'UTR", SOI$dbSNP_functional_annotation)
SOI$dbSNP_functional_annotation <- sub(pattern = "^U5$", "5'UTR", SOI$dbSNP_functional_annotation)
table <- table(SOI$dbSNP_functional_annotation)
...
# Run biomaRt
LdSNPsInfo <- getSNP(rs = LdSNPs$variation2, snpmart)
LdSNPsInfo <- LdSNPsInfo[!duplicated(LdSNPsInfo$refsnp_id),]


haplotype <- merge(x = LdSNPsInfo,
                   y = LdSNPs,
                   by.x = "refsnp_id",
                   by.y = "variation2",
                   all.x = TRUE,
...
# Select the non-coding variant by discarding the variants with an annotated CDS start
isLDSOI <- is.na(haplotype$cds_start)
# table(isSOI)
LDSOI <- haplotype[isLDSOI, ]
LDSOI$chr_name <- paste("chr", LDSOI$chr_name, sep = "")
LDSOI <- LDSOI[, c("chr_name", "chrom_start", "chrom_end","refsnp_id",  "variation1")]
#View(LDSOI)
#LDSOI <- LDSOI[!duplicated(LDSOI$refsnp_id),]
LDSOI <- na.omit(LDSOI)


haplotypeGR <- with(LDSOI,
                    GRanges( seqnames = Rle(chr_name),
                             ranges   = IRanges(start = chrom_start, end = chrom_end),
                             strand   = Rle("*"), LDSNPs = refsnp_id, TagSNPs = variation1))

haplotype.block <- LDSOI %>% 
  dplyr::group_by(variation1) %>% 
  dplyr::mutate(Region_start = min(chrom_start),
            Region_end = max(chrom_end),
            Nb_SNPs = n())

haplotype.block <- haplotype.block[!duplicated(haplotype.block$variation1),]
#View(haplotype.block)
haplotype.blockGR <- with(haplotype.block,
                          GRanges( seqnames = Rle(chr_name),
...
```{r number_of_LDSNPs, eval=FALSE, echo=FALSE}
...
eTerm <- xEnricherSNPs(SOI$refsnp_id, ontology = c("EF", "EF_disease",
...
varScanJaspar.file <- "~/Google\ Drive/Yvon_Mbouamboua_these_2018/gwas/database/varScanJaspar.txt"
...
snpInfoGR <- with(snpInfoBed,
                   GRanges( seqnames = Rle(chr_name),
                            ranges   = IRanges(start = chrom_start, end = chrom_end),
                            strand   = Rle("*"), rsid = refsnp_id))
...
remap.intersect.SOIs <- IntersectBed(remapCatalog, snpInfoGR)
## Find intersect between remap catalog and haplor data

var.file <- "remap.tsv"
var.path <- file.path("~/cisreg-GWAS_results/EFO_0008407/", var.file)
...
# Export remap data in tsv table
remap.inter.SOIs <- merge(snpInfo,
      remap.inter.SOIs,
      by.x = c("chrom_start", "chrom_end"),
      by.y = c("start.1", "end.1"),
      all.x = FALSE,
      all.y = TRUE)
...
enrich <- enrichment(haplotype.blockGR, remapCatalog)  

# Negative control

shufflesRegion <- shuffle(haplotype.blockGR)
...
SOI.file <- "varscan.with.peaks.tsv"
nocoding.path <- file.path("~/cisreg-GWAS_results/EFO_0008407/", SOI.file)
