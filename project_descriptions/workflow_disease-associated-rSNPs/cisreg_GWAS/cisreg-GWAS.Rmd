...
gwascatalog.disease.urlrest <- paste(
  sep = "", 
  "https://www.ebi.ac.uk/gwas/rest/api/efoTraits/", parameters$query, "/associations?projection=associationByEfoTrait")
message("GWAS catalog link\t", gwascatalog.disease.urlrest)

gwascatalog.studies.urlrest <- paste(
  sep = "", 
  "https://www.ebi.ac.uk/gwas/rest/api/efoTraits/", parameters$query, "/studies")
...
gwasrestoutput <- fromJSON(gwascatalog.disease.urlrest, content_type("application/json"))
 
stop_for_status(gwasrestoutput)

numberOfRows <- nrow(df["studies.accessionId"]);
...
#head(data.frame(t(sapply(content(gwasrestoutput),c))))
gwasstruct <- (fromJSON(toJSON(content(gwasrestoutput))))

df <- data.frame(gwasstruct$`_embedded`)
