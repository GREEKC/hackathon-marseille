...
for (rsID in DArsIDs) {
  ...
  # sView(ldSNPs)
  if (length(newLdSNpIDs) > parameters$max.ld.per.snp) {
    message("Number of LD for tag SNP ", rsID, " exceeds limit (", parameters$max.ld.per.snp, "). LD SNPs are ignored for this Tag SNP. ")
  } else if (length(newLdSNpIDs) > 0) {
    lsSNPsIDs <- append(lsSNPsIDs, newLdSNpIDs)
  }
}
lsSNPsIDs <- unique(lsSNPsIDs)
nbLDSNPs <- length(lsSNPsIDs)
message("Total number of DA-SNPs and LD SNPs: ", nbLDSNPs)
#sort(table(lsSNPsIDs))
...
## biomaRt annotation
message("Gathering information about LD SNPs from Ensembl")
...
# Run biomaRt
getSNP <- getSNP(rs = lsSNPsIDs, snpmart)
getSNP <- getSNP[!duplicated(getSNP$refsnp_id),]

#dim(getSNP)

# Select the non-coding variant by discarding the CDS variants

nonCoding <- subset(getSNP,
                    is.na(cds_start))


nocoding.file <- "ensemblNonConding.tsv"
nocoding.path <- file.path("~/cisreg-GWAS_results/EFO_0008407/", nocoding.file)
write.table(x = nonCoding,
            file = nocoding.path,
            quote = FALSE,
            sep = "\t",
            row.names = FALSE,
