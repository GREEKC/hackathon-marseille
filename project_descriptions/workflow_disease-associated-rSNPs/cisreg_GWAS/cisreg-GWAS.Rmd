...
pie(SOIsConsequenceTable,
    main  = "Variant types of SNPs of interest",
    labels = lbls,
    cex = 1,
...
res <- POST(file.path(parameters$RsatRestUrl, "variation-info", "Homo_sapiens", "GRCh38"),
            body = list(i_string = input, 
                        i_string_type = 'text'))

varinfo <- httr::content(res, as = "text", encoding = "UTF-8")
varinfo.fromJSON <- fromJSON(varinfo)
#varBed <- varinfo$output

## Get the URL of the varBed file produced by variation-info, and still stored on the server
varBed.URL <- varinfo.fromJSON$result_url
varBed.server.path <- varinfo.fromJSON$result_path

## Download variation info result and store it to a local file
varBed.file <- file.path(
  result.dir.path["RSAT"], 
  paste(sep = "", parameters$query, "_SOIs_info", ".varbed"))
download.file(url = varBed.URL, destfile = varBed.file)
...
varSeq <- POST(file.path(parameters$RsatRestUrl, "retrieve-variation-seq", "Homo_sapiens", "GRCh38"),
               body = list(i_string = varBed.server.path,
...
## Variation-scan
message("Scanning the different alleles of a variant with motifs via RSAT REST services")
