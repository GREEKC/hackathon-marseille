...
soi.file <- "SOIs.txt"
soi.path <- file.path("~/cisreg-GWAS_results/EFO_0001068/SOIs", soi.file)
...
table <- table(SOIs$consequence_type_tv)
pct <- round(table/sum(table)*100)
lbls <- names(table)
lbls <- paste(lbls, pct) # add percents to labels
lbls <- paste(lbls, "%", sep = "") # ad % to labels
...
## Variation-info
message(" retrieval of information of Ensembl annotated variants")

res <- POST("http://rsat-tagc.univ-mrs.fr/rsat/rest/variation-info/Homo_sapiens/GRCh38",
            body=list( i_string = input, 
                       i_string_type='text'))
          
varinfo <- content(res, "text", encoding = "UTF-8")
varinfo <- fromJSON(varinfo)
#varBed <- varinfo$output
varBed <- varinfo$result_path

## Retrieve variation sequence

message("retrieval of the sequences surrounding variants")

seq <- POST("http://rsat-tagc.univ-mrs.fr/rsat/rest/retrieve-variation-seq/Homo_sapiens/GRCh38",
            body=list(i_string=varBed, 
...
message("scanning of different alleles of a variant with one or several motifs")
varscan <- POST("http://rsat-tagc.univ-mrs.fr/rsat/rest/variation-scan/Homo_sapiens/GRCh38", 
                body = list(
                  i_input_string = varSeq,
                  i_input_string_type = 'piping',
                  m = upload_file("~/Drive/Yvon_Mbouamboua_these_2018/gwas/database/JASPAR2018_CORE_vertebrates_non-redundant_pfms_transfac.txt"),
                  m_format =  "transfac"),
...
variationScan <- read.delim(file = "~/cisreg-GWAS_results/EFO_0008407/variation-scan.tsv",
                            header = TRUE,
                            sep = "\t",
                            comment.char = ";")
...
varScanJaspar.file <- "~/Drive/Yvon_Mbouamboua_these_2018/gwas/database/variation-scan_2019-05-06_malaria_pvalE-4_pvalRatio100.txt"
...
results_path = "~/cisreg-GWAS_results/EFO_0001068/JASPAR2018/"
