...
bed_to_granges <- function(file){
  ...
  header <- c('chr','start','end','ID','name','strand')
  ...
  if(length(df)==3){
    ...
    gr <- with(df, GRanges(chr, IRanges(start, end), ID=ID, name=name))
  } else if (length(df)==6){
    gr <- with(df, GRanges(chr, IRanges(start, end), ID=ID, name=name, strand=strand))
  }
  return(gr)
}